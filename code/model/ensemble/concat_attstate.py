...
# and concat attented state and output of tsl
#
# 1. attention net 
#   - multiply attention and state
#
# 2. concatenate attented state and output of tsl model 
#
...
class MEL_concat_Attstate():
    ...
    def build_graph(self, args, dim_feature, dim_state):
        ...
        with self.graph.as_default():
            ...
            # multiply state and attention
            att_state_A = tf.multiply(att_A, self.state_A)
            att_state_B = tf.multiply(att_B, self.state_B)
            att_state_C = tf.multiply(att_C, self.state_C)
            
            # 2. concatenate attented state and value of tsl model
            concat_A = tf.concat([self.value_A, att_state_A], 1)
            concat_B = tf.concat([self.value_B, att_state_B], 1)
            concat_C = tf.concat([self.value_C, att_state_C], 1)
