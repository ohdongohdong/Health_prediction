...
from model.state_model.mel_base import MEL_base_model
from model.state_model.mel_attention import MEL_attention_model
from model.state_model.mel_concat_base import MEL_concat_base
from model.state_model.mel_concat_attstate import MEL_concat_Attstate
from model.state_model.mel_concat_attention import MEL_concat_attention

# flags
from tensorflow.python.platform import flags
flags.DEFINE_string('mode', 'train', 'select train or test')
flags.DEFINE_string('model', 'hierarchy', 'select  model attention, baseline, concat_base, concat_attstate, concat_attention')
...
class Runner(object):
        ...
        with tf.Session(graph=model.graph, config=get_tf_config()) as sess: 
            ...
            for each_epoch in range(epoch):
                ...
                for b in range(batch_epoch):
                    ...
                    p, t, s = sess.run([model.predict,model.targets,
                                        model.output_states],
                                        feed_dict=feed)
    ...
    def train(self, args, model,
                input_train_A, input_train_B, input_train_C,
        ...
        with tf.Session(graph=model.graph, config=get_tf_config()) as sess:
            ...
            for each_epoch in range(epoch):
                ...
                for b in range(batch_epoch):
                    ...
                    batch_inputs_A, batch_inputs_B, batch_inputs_C,\
    ...
    def test(self, args, model,
            input_test_A, input_test_B, input_test_C,
            state_test_A, state_test_B, state_test_C, target_test):
    ...
    def run(self):
        ...
        input_A, target_set, state_A= self.load_tsl(args, tsl_A_model, input_set, target_set, seq_len_set)
        
        args.model = 'B'
        tsl_B_model = TSL_model(args, num_steps, dim_inputs, dim_targets)
        input_B, _, state_B = self.load_tsl(args, tsl_B_model, input_set, target_set, seq_len_set)
        
        args.model = 'C'
        tsl_C_model = TSL_model(args, num_steps, dim_inputs, dim_targets)
        input_C, _, state_C = self.load_tsl(args, tsl_C_model, input_set, target_set, seq_len_set)  
        ...
        if model == 'baseline':
            mel_model = MEL_base_model(args, dim_feature, dim_state)
        elif model == 'attention':
            mel_model = MEL_attention_model(args, dim_feature, dim_state)
